...
# define EMAIL_TOKEN_MAX_SIZE 50
// 邮件内容结构
struct UploadData {
    const char *data;
    size_t size;
...
// 这里 token 需要追加'\0'
int _send_email(const char * recipient, const char * token,size_t token_size){ 
    ...
    if(curl) {
        ...
        sprintf(email_data,email_template,FROM_EMAIL,recipient,token);
...
int send_email(const char * recipient, const char * token,size_t token_size){
    // _send_email(recipient,token,token_size);
    return 0;
}
