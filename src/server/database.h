...
int _query_database(MYSQL * my_connection, char *sql,size_t counts,...){
    ...
    const unsigned char ** result_data_pp = va_arg(args, const unsigned char **);
    size_t * result_data_size_p = va_arg(args, size_t *);
    enum_field_types result_data_type=va_arg(args,enum_field_types);

    va_end(args);

    unsigned char * _result_data=OPENSSL_zalloc(DB_BUFFER_MAX_SIZE);
...
// query
/* 在 register request callback 阶段查询 user 提交的 user_name 是否已被注册 */

int validate_username(MYSQL* my_connection, const char * user_name, size_t username_size){
    ...
    if (memcmp(user_name,ret_username,ret_username_size)){
        return 1; // 帐号已被注册
    }
...
/* 用于在 update&query request callback 阶段，server 检查 token 是否有效 */
int validate_token(MYSQL * my_connection, 
...
int query_challenge(MYSQL * my_connection, 
    const unsigned char *session_id, size_t session_id_size, 
    const char * user_name, size_t username_size,
    const unsigned char **challenge_pp, size_t *challenge_size_p)
{   
    ...
    if (query_res==0){
        *challenge_pp=ret_challenge;
        *challenge_size_p=ret_challenge_size;
        return 0;  
    }
    return -1;
...
/* 用于在 response request callback 阶段，server 提取 pubkey */

int query_pubkey(MYSQL * my_connection, 
    const char * user_name, size_t username_size,
    EVP_PKEY ** pubkey_pp)
{
    ...
    if (query_res==0){
        const unsigned char  * uint8_pubkey;
        size_t  uint8_pubkey_size;
        hex_to_uint8(ret_pubkey, ret_pubkey_size, &uint8_pubkey, &uint8_pubkey_size);
        *pubkey_pp=EVP_PKEY_new_raw_public_key_ex(NULL,"ML-DSA-44",NULL, uint8_pubkey, uint8_pubkey_size);
        return 0;  
    }
    if (query_res==-1){ 
        return -1;
...
int query_data(MYSQL* my_connection, const char * user_name, size_t username_size, const char ** data_pp, size_t * data_size_p){
    ...
    int query_res=_query_database(my_connection,sql,2,
        user_name, &username_size, MYSQL_TYPE_STRING, 
        data_pp, data_size_p, MYSQL_TYPE_STRING);
...
int query_email(MYSQL* my_connection, const char * user_name, size_t username_size, const char ** email_pp, size_t * email_size_p){
...
int validate_change_factor_token(MYSQL * my_connection, 
    const unsigned char *session_id, size_t session_id_size, 
    const char * user_name, size_t username_size,
    const unsigned char * change_factor_token, size_t change_factor_token_size)
{   
...
int validate_email_token(MYSQL * my_connection, 
    const unsigned char *session_id, size_t session_id_size, 
    const char * user_name, size_t username_size,
    const unsigned char * email_token, size_t email_token_size)
    {
...
// record 
int record_email_token(MYSQL* my_connection,
    const unsigned char * session_id, size_t session_id_size,
    const char * user_name, size_t username_size,
    ...
    sprintf(sql,"INSERT INTO %s (session_id, user_name, email, email_token_timestamp, email_token) VALUES(?,?,?,?,?)",TAB_REGISTER_SESSION);


    const char * datetime=get_datetime_str();
    size_t datetime_size=19;   
    int update_res=_update_database(my_connection,sql,5,
        session_id, &session_id_size, MYSQL_TYPE_STRING,
        user_name, &username_size, MYSQL_TYPE_STRING,
...
int record_register(MYSQL* my_connection,
    const char * user_name, size_t username_size,
    const char * email, size_t email_size,
...
int record_login(MYSQL* my_connection,
    const unsigned char * session_id, size_t session_id_size,
    const char * user_name, size_t username_size,
...
int record_response(MYSQL* my_connection,
    const unsigned char * session_id, size_t session_id_size,
    const char * user_name, size_t username_size,
...
int record_token(MYSQL* my_connection,
    const unsigned char * session_id, size_t session_id_size,
    const char * user_name, size_t username_size,
    const unsigned char * token, size_t token_size)
{
...
int update_data(MYSQL* my_connection,
    const char * user_name, size_t username_size,
    const char * new_data, size_t new_data_size)
{    
    char *sql=(char *)malloc(SQL_MAX_LEN);
...
int update_pubkey(MYSQL* my_connection,
    const char * user_name, size_t username_size,
...
int record_change_factor_token(MYSQL* my_connection,
    const unsigned char * session_id, size_t session_id_size,
    const char * user_name, size_t username_size,
    const unsigned char * token, size_t token_size)
{
